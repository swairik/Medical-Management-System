...
public class CredentialServiceTest {
    ...
    void testCreateCredentials() {
        cred = Credential.builder().email(genAlnum(10) + "@xyz.com").password("xyz")
                        .role(Role.ADMIN).build();
        assertThat(impl.create(cred)).isEqualTo(cred);
    }
    ...
    void testGetCredentialsByEmail() {
        assertThat(impl.getByEmail(cred.getEmail())).isNotEmpty().contains(cred);
    }
    ...
    void testGetCredentialsById() {
        assertThat(impl.get(cred.getId())).isNotEmpty().contains(cred);
    }
    ...
    void testUpdateCredentials() {
        ...
        assertThat(impl.update(cred.getId(), tempCreds)).isEqualTo(tempCreds).isNotEqualTo(cred);
    }

    @Test
    void testDeleteCredentials() {
        impl.delete(cred.getId());
        assertThat(impl.get(cred.getId())).isEmpty();
