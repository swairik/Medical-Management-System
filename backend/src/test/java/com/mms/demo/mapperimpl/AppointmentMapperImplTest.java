...
public class AppointmentMapperImplTest {
    @Autowired
    DataTransferObjectMapper<Appointment, AppointmentDTO> mapper;

    @Test
    void testDtoToEntity() {
        final AppointmentDTO appointmentDTOtest =
                        AppointmentDTO.builder().appointmentDetails(null).build();
        assertThatIllegalArgumentException()
                        .isThrownBy(() -> mapper.dtoToEntity(appointmentDTOtest));

        // AppointmentDTO appointmentDTO =
        // AppointmentDTO.builder().appointmentDetails(AppointmentDetailsDTO.builder().feedback("").prescription("").build()).doctor(DoctorDTO.bu)
    }
